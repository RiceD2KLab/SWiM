"""
    This script generates masks for the PoseBowl dataset using the SAM2 model. 

    Args:
        source_dir (str): The source directory containing the PoseBowl dataset with the bounding box labels.
        dest_dir (str): The destination directory to save the segmentation masks + images.
        split (str): The split to process (train, test, val).
        sam_size (str): The size of the SAM2 model to use (t, s, b, l).
    
    Results:
        The masks are saved in the masks directory @ dest_dir.
        The images are copied to the images directory @ dest_dir.
"""
...
from consts import POSEBOWL_OBJDET_DIR, DATA_DIR, MODELS_DIR
...
def main(source_dir: str, dest_dir: str, split: str, sam_size: str) -> None:
    input_dir = DATA_DIR / source_dir
    output_dir = DATA_DIR / dest_dir

    train_ids, test_ids, val_ids = setup(input_dir, output_dir)

    logger.debug(f"Found {len(train_ids)} train images")
    logger.debug(f"Found {len(test_ids)} test images")
    logger.debug(f"Found {len(val_ids)} val images")

    train_df, test_df, val_df = get_dfs(output_dir, train_ids, val_ids, test_ids)

    sam = SAM(MODELS_DIR / f"sam2_{sam_size}.pt")
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--source_dir",
        type=str,
        default="posebowl_objdet",
        help="The source directory",
    )
    argparser.add_argument(
        "--dest_dir",
        type=str,
        default="posebowl_segmented",
    ...
    argparser.add_argument(
        ...
        type=str,
        default="b",
        help="The size of the SAM2 model to use (t, s, b, l)",
    )
    args = argparser.parse_args()
    main(args.source_dir, args.dest_dir, args.split, args.sam_size)
