...
def create_mask_from_polygons(image_shape, polygons):
    ...
    for poly in polygons:
        ...
        poly = np.array(poly) * np.array(
            image_shape[::-1]
        )  # Reverse shape for (height, width)
        rr, cc = polygon(
            poly[:, 1], poly[:, 0], mask.shape
        )  # Note: y,x order for polygon
        ...
        for image_name in os.listdir(args.image_dir):
            if image_name.endswith(
                ...
                txt_file_path = os.path.join(
                    args.txt_dir, os.path.splitext(image_name)[0] + ".txt"
                )

                if not os.path.exists(txt_file_path):
                    print(
                        f"Warning: No corresponding TXT file found for {image_name}. Skipping."
                    )
                    continue
                ...
                print(
                    f"Processed {image_name}: Dice={dice_score:.4f}, Hausdorff={hausdorff_dist:.4f}"
                )
