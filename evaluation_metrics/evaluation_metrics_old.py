...
class Evaluator:
    ...
    def hausdorff_distance_contours(
        ground_mask: np.ndarray, pred_mask: np.ndarray
    ) -> float:
        """
    ...
    def evaluate(self, images_dir, txt_dir, preds_dir="./", metrics="all"):
        ...
        if metrics == "all":
            ...
            # Throw error
            raise ValueError(
                "Please enter a valid metric string from: 'dice', 'hausd' or 'all'."
            )
                ...
                for file in imgs_dir:
                    if file.is_file() and file.name.endswith(".png"):
                        ...
                        txt_path = os.path.join(
                            txt_dir, os.path.basename(file.path).split(".")[0] + ".txt"
                        )
                        image, ground_truth_mask = self.binary_mask_to_yolo_poly(
                            result.segments
                        )
                        prediction_mask = eval.yolo_predictions_to_masks(
                            [result], img.shape
                        )

                        if get_dice:
                            dice_coeff = eval.dice_coefficient(
                                ground_truth_mask, prediction_mask
                            )
                            instance_dice_coefficients.append(dice_coeff)
                        if get_hausd:
                            hd = eval.hausdorff_distance_contours(
                                ground_truth_mask, prediction_mask
                            )
